"""Bundled AI provider — runs Qwen2.5-Coder-0.5B locally via llama.cpp.
...
Tradeoffs are deliberately honest:
  ...
  - Quality: a 0.5B coder model is plausible on the bundled sample
    notebook (small schemas, easy joins) but will clearly degrade on
    real warehouses. We surface OpenAI / Anthropic / Ollama-with-a-
    bigger-model as the upgrade path. Picked this size specifically
    because HF Space's free-tier shared 2 vCPU couldn't keep up with
    the 1.5B we shipped first — 30-90 s per call felt broken.
  - Speed: ~60-100 tok/sec on a modern laptop CPU → ~2-4 s per SQL
    gen. On HF Space's 2 vCPU, ~8-20 s. On Apple Silicon Metal or a
    consumer GPU, ~instant.
  - Memory: Q4_K_M quant uses ~600 MB RAM.
...
# Sensible defaults for a 0.5B coder model on commodity CPU. Tuned for
...
# without rebuilding the image — e.g., point QWEN_MODEL_PATH at a
# downloaded Qwen 7B or Phi-3 if they have GPU and want quality.
DEFAULT_MODEL_PATH = "/app/models/qwen2.5-coder-0.5b-instruct-q4_k_m.gguf"
...
class BundledAIProvider(AIProvider):
    """Local-inference AI provider over Qwen2.5-Coder-0.5B-Instruct."""
    ...
    def generate_infographic_brief(
        ...
        return InfographicBrief(
            ...
            caveats=[
                "Generated by the bundled local model (Qwen2.5-Coder-0.5B). "
